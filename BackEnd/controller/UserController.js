...
export const registerUserController = async (req, res) => {
  try {
    const { email, name, password, role , openTo } = req.body;

    if (!email || !name || !password || !role || !openTo) {
...
// controller to add about section
export const addAboutController = async (req, res) => {
  try {
    ...
    if (!email || !about) {
      return res.status(400).send({
        success: false,
        message: "Email or about missing",
      });
    }
